...
When a turn is started from Neovim (`<leader>ai`) and any pi extension raises a blocking prompt (`ctx.ui.select` / `ctx.ui.confirm` / a custom `ctx.ui.custom` component such as pi-permission-system's permission dialog), pi-bridge surfaces it in Neovim. `select` / `confirm` are fully answerable from either surface (first answer wins; the other cleans itself up); `custom` prompts are answered in pi — Neovim shows a passive notice with an Esc-to-abort escape hatch.
